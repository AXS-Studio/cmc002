...
<head>
		...
		<link rel="stylesheet" media="screen" href="css/timeline.css"><!-- cindy -->
...
var margin = {top: 10, right: 20, bottom: 200, left: 30},
 ...
 var margin3 = {top: 510, right: 10, bottom: 20, left: 40};
		...
		for (var i = 0; i < initialData.length; i++) {
			if (initialData[i].id != 'comment' && initialData[i].id != 'tags' && initialData[i].id != 'notes' && initialData[i].id != 'sessions'){
				...
				if (initialData[i].results!=null){
					...
					x.domain(d3.extent(initialData[i]["results"], function(d) { return d.date;}));
					y.domain([0, 100]);
